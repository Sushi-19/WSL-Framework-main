...
from .baseline import SimpleCNN, ResNet, MLP
...
class NoiseRobustModel(nn.Module):
    ...
    def __init__(
        ...
        if model_type == 'simple_cnn':
            ...
            self.model1 = MLP(input_size=3072, num_classes=num_classes)
        else:
        ...
        if use_co_teaching:
            if model_type == 'simple_cnn':
                ...
                self.model2 = MLP(input_size=3072, num_classes=num_classes)
            else:
                raise ValueError(f"Unsupported model type: {model_type}")
